...
public class clsProductoDAO {
     ...
     public static int Agregar(Context context,clsProducto entidad)
    {
     ...
     public static  List<clsProducto> ListarEmpresa(Context context,int idMarca,int idTipoProducto,boolean empresa)
     {
        List<clsProducto> list=new ArrayList<clsProducto>();
        bdSQLite admin=new bdSQLite(context,null); 
        SQLiteDatabase bd=admin.getWritableDatabase();
         if(bd!=null)
         {
              String query="select p.int_id_producto,p.str_nombre,p.dat_fecha_actualizacion,p.int_id_tipo_producto,"
                    ...
                    if(empresa)
                    {
                        query+=" where pe.int_id_producto_empresa IS not NULL";
                        if(idMarca>0 && idTipoProducto>0)
                        {
                            query+=" and p.int_id_marca="+idMarca+" and p.int_id_tipo_producto="+idTipoProducto;
                        }
